...
import com.sailmi.mall.foundation.domain.GoodsClassStaple;
import com.sailmi.mall.foundation.domain.GoodsType;
import com.sailmi.mall.foundation.domain.query.GoodsClassQueryObject;
import com.sailmi.mall.foundation.service.IAccessoryService;
import com.sailmi.mall.foundation.service.IGoodsClassService;
import com.sailmi.mall.foundation.service.IGoodsClassStapleService;
...
public class GoodsClassManageAction {
	...
	public void goods_class_ajax(HttpServletRequest request, HttpServletResponse response, String id, String fieldName,
		...
		BeanWrapper wrapper = PropertyAccessorFactory.forBeanPropertyAccess(gc);
		Object val = null;
			...
			if (!id.equals("")) {
				...
				try {
					...
					for (GoodsClass gc : gcs) {
						Map gcparams = new HashMap();
						gcparams.put("gcid", gc.getId());
						List<Goods> glist = this.goodsService.query("select obj from Goods obj where obj.gc.id=:gcid",
								gcparams, -1, -1);
						for (Goods goods : glist) {
							goods.setGc(null);
							this.goodsService.update(goods);
						}
						List<GoodsClassStaple> gscList=this.goodsClassStapleService.query("select obj from GoodsClassStaple obj where obj.gc.id=:gcid", gcparams, -1,-1);
						for (GoodsClassStaple gsc : gscList) {
