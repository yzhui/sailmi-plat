package com.sailmi.mall.lucene;
...
public class LuceneUtil {
	...
	public List<LuceneVo> searchIndex(String keyword, int start, int size, double begin_price, double end_price, Sort sort) throws IOException {
		...
		try {
			...
			//reader = IndexReader.open(FSDirectory.open(index_file.toPath()).getDirectory());
			searcher = new IndexSearcher(reader);
			IKSimilarity iks=new IKSimilarity();
			//searcher.setSimilarity(iks);
	...
	*/
	...
	public void update(String id, LuceneVo vo) {
		try {
			index_file = new File(index_path);
			Directory directory = FSDirectory.open(index_file.toPath());
	...
	public void deleteAllIndex(boolean isdeletefile) {
		...
		if ((index_file.exists()) && (index_file.isDirectory())) {
			try {
				reader = DirectoryReader.open(FSDirectory.open(index_file.toPath()));
	...
	private IndexWriter openIndexWriter() throws IOException {
		...
		IndexWriter writer = new IndexWriter(FSDirectory.open(index_file.toPath()), indexWriterConfig);
