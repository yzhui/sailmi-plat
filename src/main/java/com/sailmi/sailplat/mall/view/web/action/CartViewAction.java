...
import com.sailmi.sailplat.manage.admin.tools.MsgTools;
import com.sailmi.sailplat.manage.admin.tools.PaymentTools;
import com.sailmi.sailplat.manage.seller.Tools.TransportTools;
...
import com.sailmi.tools.QRCodeEncoderHandler;
import com.sailmi.tools.WebForm;
import com.sailmi.tools.WxAdvancedUtil;
import com.sailmi.tools.WxCommonUtil;
import com.sailmi.tools.bean.WxOauth2Token;
import com.sailmi.tools.bean.WxToken;
...
public class CartViewAction {
	...
	private List<StoreCart> cart_calc(HttpServletRequest request) {
		...
		System.out.println("211111111111111111111111111111111");
		...
		System.out.println("311111111111111111111111111111111");
		if (user != null) {
			...
			System.out.println("411111111111111111111111111111111");
		...
		System.out.println("511111111111111111111111111111111");
	 ...
	 * mobile支付提交
	 */
	@SecurityMapping(display = false, rsequence = 0, title = "mobile订单支付", value = "/wxmobile_submit.htm*", rtype = "buyer", rname = "购物流程3", rcode = "goods_cart", rgroup = "在线购物")
	...
	public String paymentSubmit(HttpServletRequest request,
		...
		OrderForm of = this.orderFormService.getObjById(CommUtil.null2Long(order_id));
		String subject = "订单号为" + of.getOrder_id();
		String out_trade_no = of.getId().toString();
		String body = "支付宝mobile支付";
	    String product_code="QUICK_WAP_PAY";
	    String timeout_express="2m";
	    String notify_url=siteURL + "/alipay/alipay_notify.htm";
	    String return_url=siteURL + "/alipay/alipay_retrun.htm";
	    String total_amount=of.getGoods_amount().toString();
		if (of != null && of.getOrder_status() == 10) {
			...
			// 支付方式已经配置:mobile支持支付宝mobile支付以及微信公众号支付
			if (payments.size() > 0) {
				...
				this.orderFormService.update(of);
				// 微信公众号支付
				if (payType.equals("weixin_mobile")) {
				...
				} else if (payType.equals("alipay_mobile")) {
					...
					// 调用的接口名，无需修改
					sParaTemp.put("service", "alipay.mobile.create.direct.pay.by.user");
					...
					sParaTemp.put("body", "支付宝mobile支付");
					// 其他业务参数根据在线开发文档，添加参数.文档地址:https://doc.open.alipay.com/doc2/detail.htm?spm=a219a.7629140.0.0.2Z6TSk&treeId=60&articleId=103693&docType=1
			        
					String sHtmlText = AlipaySubmit.buildRequestmobile(config, sParaTemp, "get", "确定");
	...
	public String cart_address_save(HttpServletRequest request, HttpServletResponse response, String id, String area_id, String store_id) {
		...
		if (id.equals("")){
			address.setId(null);//主锓ID为0，会导致detached错误
