package com.sailmi.sailplat.manage.timer;
...
public class StatManageAction {
	...
	public void execute() throws Exception {
		...
		for (Store store : stores) {
			...
			if(evas.size()>0) description_evaluate = CommUtil.null2Double(df.format(description_evaluate_total / evas.size()));
	...
	private boolean send_email(OrderForm order, String mark) throws Exception {
		...
		if (template.isOpen()) {
			...
			String path = System.getProperty("sailplat.root") + "vm" + File.separator;
			...
			p.setProperty("file.resource.loader.path", System.getProperty("sailplat.root") + "vm" + File.separator);
