...
import com.sailmi.annotation.Log;
...
import com.sailmi.sailplat.manage.admin.tools.StatTools;
 ...
 public class BaseManageAction
 {
     ...
     throws IOException
   {
     ...
     if (this.configService.getSysConfig().isSecond_domain_open()) {
       Cookie tbl_user_session = new Cookie("tbl_user_session", user.getId().toString());
       tbl_user_session.setDomain(CommUtil.generic_domain(request));
       response.addCookie(tbl_user_session);
     }
   ...
   public void logout_success(HttpServletRequest request, HttpServletResponse response) throws IOException {
     ...
     if (this.configService.getSysConfig().isSecond_domain_open()) {
       ...
       for (Cookie cookie : cookies) {
         if (cookie.getName().equals("tbl_user_session")) {
           cookie.setMaxAge(0);
           cookie.setValue("");
   ...
   public ModelAndView login_error(HttpServletRequest request, HttpServletResponse response) {
     ...
     if ((tbl_view_type != null) && (!tbl_view_type.equals(""))) {
       if (tbl_view_type.equals("mobile")) {
