...
import com.sailmi.sailplat.foundation.service.IResService;
import com.sailmi.sailplat.foundation.service.IRoleGroupService;
...
import com.sailmi.sailplat.mall.view.web.action.CartViewAction;
import com.sailmi.sailplat.manage.buyer.action.BaseBuyerAction;
import com.sailmi.sailplat.manage.seller.action.BaseSellerAction;
import com.sailmi.sailplat.security.SecurityManager;
...
@Controller
public class EnterpriseManageAction implements ServletContextAware {
	...
	@SecurityMapping(display = false, rsequence = 0, title = "管理员列表", value = "/admin/enterprise_list.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置")
	@RequestMapping({ "/admin/enterprise_list.htm" })
	public ModelAndView enterprise_list(String currentPage, String orderBy, String orderType, HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/enterprise_list.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
		...
		CommUtil.saveIPageList2ModelAndView(url + "/admin/enterprise_list.htm", "", "", pList, mv);
		mv.addObject("userRole", "ADMIN");
		return mv;
	}

	@SecurityMapping(display = false, rsequence = 0, title = "管理员添加", value = "/admin/enterprise_add.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置")
	@RequestMapping({ "/admin/enterprise_add.htm" })
	public ModelAndView enterprise_add(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/enterprise_add.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
	...
	@SecurityMapping(display = false, rsequence = 0, title = "管理员编辑", value = "/admin/enterprise_edit.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置")
	@RequestMapping({ "/admin/enterprise_edit.htm" })
	public ModelAndView enterprise_edit(HttpServletRequest request, HttpServletResponse response, String id, String op) {
		ModelAndView mv = new JModelAndView("admin/blue/enterprise_add.html", this.configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
		Map params = new HashMap();
		params.put("type", "ADMIN");
		List rgs = this.roleGroupService.query("select obj from RoleGroup obj where obj.type=:type order by obj.sequence asc", params, -1, -1);
	...
	@SecurityMapping(display = false, rsequence = 0, title = "管理员保存", value = "/admin/enterprise_save.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置")
	@RequestMapping({ "/admin/enterprise_save.htm" })
	public ModelAndView enterprise_save(HttpServletRequest request, HttpServletResponse response, String id, String role_ids, String list_url, String add_url, String password) {
		/*WebForm wf = new WebForm();
		User user = null;
		if (id.equals("")) {
	...
	@SecurityMapping(display = false, rsequence = 0, title = "管理员删除", value = "/admin/enterprise_del.htm*", rtype = "admin", rname = "管理员管理", rcode = "admin_manage", rgroup = "设置")
	@RequestMapping({ "/admin/enterprise_del.htm" })
	public String enterprise_del(HttpServletRequest request, String mulitId, String currentPage) {
		String[] ids = mulitId.split(",");
		for (String id : ids) {
			if (!id.equals("")) {
				User user = this.userService.getObjById(Long.valueOf(Long.parseLong(id)));
				if (!user.getUsername().equals("admin")) {
					this.databaseTools.execute("delete from tbl_syslog where user_id=" + id);
					this.databaseTools.execute("delete from tbl_user_role where user_id=" + id);
					this.userService.delete(user.getId());
				}
			}
		}
		return "redirect:enterprise_list.htm?currentPage=" + currentPage;
	}
	...
	public String init_role() {
		...
		if ((current_user != null)  && (current_user.getUserRole().equals("admin"))) {
			this.databaseTools.execute("delete from tbl_role_res");
			this.databaseTools.execute("delete from tbl_res");
			this.databaseTools.execute("delete from tbl_user_role");
			this.databaseTools.execute("delete from tbl_role");
			this.databaseTools.execute("delete from tbl_rolegroup");
