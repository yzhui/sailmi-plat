 package com.sailmi.sailplat.pay.alipay.util;
...
import com.sailmi.sailplat.pay.alipay.util.httpClient.HttpProtocolHandler;
import com.sailmi.sailplat.pay.alipay.util.httpClient.HttpRequest;
import com.sailmi.sailplat.pay.alipay.util.httpClient.HttpResponse;
import com.sailmi.sailplat.pay.alipay.util.httpClient.HttpResultType;
 
 public class AlipaySubmit
 {
   private static final String ALIPAY_GATEWAY_NEW = "https://mapi.alipay.com/gateway.do?";
   private static final String mobile_ALIPAY_GATEWAY_NEW = "http://mobilepaygw.alipay.com/service/rest.htm?";
   ...
   public static String buildRequest(AlipayConfig config, String type, Map<String, String> sParaTemp, String strParaFileName, String strFilePath)
     ...
     if (type.equals("mobile")) {
       request.setUrl("http://mobilepaygw.alipay.com/service/rest.htm?_input_charset=" + 
         config.getInput_charset());
     }
     HttpResponse response = httpProtocolHandler.execute(request, 
   ...
   /**
    * mobile建立请求，以表单HTML形式构造（默认）
    * @param sParaTemp 请求参数数组
    * @param strMethod 提交方式。两个值可选：post、get
    * @param strButtonName 确认按钮显示文字
    * @return 提交表单HTML文本
    */
   public static String buildRequestmobile(AlipayConfig config, Map<String, String> sParaTemp, String strMethod, String strButtonName) {
       //待请求参数数组
       Map<String, String> sPara = buildRequestParamobile(config, sParaTemp);
    ...
    * @return 要请求的参数数组
    */
   private static Map<String, String> buildRequestParamobile(AlipayConfig config, Map<String, String> sParaTemp) {
       //除去数组中的空值和签名参数
   ...
   private static Map<String, String> buildRequestPara(AlipayConfig config, Map<String, String> sParaTemp)
   {
     ...
     sPara.put("sign", mysign);
     if (!((String)sPara.get("service")).equals("alipay.mobile.trade.create.direct"))
     {
       if (!((String)sPara.get("service")).equals(
         "alipay.mobile.auth.authAndExecute"))
         sPara.put("sign_type", config.getSign_type());
     }
     return sPara;
   }
    ...
    */
	public static String buildRequestMysign(AlipayConfig config, Map<String, String> sPara) {
       ...
       if(config.getSign_type().equals("RSA") ){
    	   mysign = RSA.signmobile(prestr, AlipayConfig.getPrivate_key(), config.getInput_charset());
       }
       return mysign;
   }
