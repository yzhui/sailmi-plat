 package com.sailmi.sailplat.core.tools;
       ...
       for (int j = 0; j < maps.size(); j++) {
         ...
         while (keys.hasNext()) {
           ...
           if (key.equals(propertys[i].getName())) {
             ...
             try {
               propertyValue = ConvertUtils.convert(map
                 .get(key), propertys[i]
                 .getPropertyType());
             }
             catch (Exception e) {
           ...
           if (field.indexOf("q_") == 0) {
             ...
             for (PropertyDescriptor pd : propertys){
                 if (pd.getName().equals(field.substring(2)))
                     para = ConvertUtils.convert(map.get(field), pd
                       .getPropertyType());
             ...
             if (field.indexOf(".") > 0) {
               ...
               entity_wrapper = PropertyAccessorFactory.forBeanPropertyAccess(entity);
               entity_propertys = entity_wrapper
                 .getPropertyDescriptors();
               for (PropertyDescriptor pd : entity_propertys) {
                 if (!pd.getName().equals(propertyName))
                   continue;
                 BeanWrapper many_to_one_entity = PropertyAccessorFactory.forBeanPropertyAccess(
                   pd.getPropertyType());
